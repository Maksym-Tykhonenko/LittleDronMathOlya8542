...
import { loadSavedTipIds, saveTipIds } from './src/storage/storage';
...
const [stage, setStage] = useState('onboarding');
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const ids = await loadSavedTipIds();
        if (mounted) {
          setSavedIds(ids);
        }
      } finally {
  ...
  }, [loadingDone, storageReady]);

  const finishOnboarding = useCallback(() => {
    setStage('main');
  }, []);

  const persistSaved = useCallback(async (nextIds) => {
    setSavedIds(nextIds);
  ...
  if (stage === 'loading') {
    return <LoadingScreen onFinish={() => setLoadingDone('')} />;
  }
